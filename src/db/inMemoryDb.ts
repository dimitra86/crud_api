...
if (!cluster.isMaster) {
  ...
  process.on('message', (msg: any) => {
    ...
    if (msg.type === 'state' && Array.isArray(msg.state)) {
      ...
      if (action === 'create' || action === 'update') users.set(payload.id, payload);
...
export const db = {
  ...
  async create(user: User): Promise<User> {
    ...
    if (!cluster.isMaster) process.send && process.send({ type: 'create', payload: user });
    return user;
  },
  async update(id: string, user: User): Promise<User | null> {
    if (!users.has(id)) return null;
    users.set(id, user);
    if (!cluster.isMaster) process.send && process.send({ type: 'update', payload: user });
    return user;
  },
  async delete(id: string): Promise<boolean> {
    const res = users.delete(id);
    if (!cluster.isMaster && res) process.send && process.send({ type: 'delete', payload: { id } });
    return res;
  },
