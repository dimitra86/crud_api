...
if (cluster.isMaster) {
  ...
  const workers: { port: number; proc: Worker }[] = [];
    ...
    worker.on('message', (msg: any) => {
      ...
      if (msg.type === 'getState') {
        ...
        for (const w of Object.values(cluster.workers || {})) {
          w?.send({ type: 'stateUpdate', action: 'create', payload: msg.payload });
        }
      } else if (msg.type === 'update') {
        dbState.set(msg.payload.id, msg.payload);
        for (const w of Object.values(cluster.workers || {})) {
          w?.send({ type: 'stateUpdate', action: 'update', payload: msg.payload });
        }
      } else if (msg.type === 'delete') {
        dbState.delete(msg.payload.id);
        for (const w of Object.values(cluster.workers || {})) {
          w?.send({ type: 'stateUpdate', action: 'delete', payload: msg.payload });
        }
      } else if (msg.type === 'clear') {
        dbState.clear();
        for (const w of Object.values(cluster.workers || {})) {
          w?.send({ type: 'stateUpdate', action: 'clear' });
        }
      }
    });
  }
  ...
  server.listen(PORT, () => {
    console.log(
      `Load balancer listening on port ${PORT}, forwarding to workers on ports ${workers
        .map((w) => w.port)
        .join(', ')}`
    );
  });
